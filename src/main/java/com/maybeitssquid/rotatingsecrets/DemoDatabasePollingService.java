 ...
 *
 * <p>Runs two polling threads that check out connections from the pool, execute a simple query, and
 * print results to stdout. This exercises the connection pool and credential rotation behavior.
 */
@Service
public class DemoDatabasePollingService {
  ...
  private void executeQuery(String threadName) {
    try (Connection conn = dataSource.getConnection();
      ...
      DemoQueryResult result = new DemoQueryResult(threadName, Instant.now(), dbTime);
